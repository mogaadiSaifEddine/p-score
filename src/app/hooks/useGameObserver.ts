...
import { gameObserverService, GameByCodeResponse, ObserverData, ScoreboardData } from '../lib/game-observer-service';
...
export function useGameObserver({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({
        ...
        teamScoreboard: teamScoreboardResponse?.success ? teamScoreboardResponse.data : prev.teamScoreboard,
        loading: false,
        error: null
      }));
  ...
  const getTeamById = useCallback((targetTeamId: number) => {
    ...
    if (state.teamScoreboard) {
      ...
      if (state.teamScoreboard.teams && Array.isArray(state.teamScoreboard.teams)) {
        const team = state.teamScoreboard.teams.find(team => team.game_team_id === targetTeamId);
        if (team) {
  ...
  return {
    ...
    getTeamTreasures: () => {
      if (state.teamScoreboard) {
        ...
        return teamData.treasures_found || 
               teamData.treasures || 
