...
export class RouteParser {
  private static defaultRules: RouteValidationRules = {
    ...
    validLanguages: ['en', 'es', 'fr', 'de', 'it', 'pt', 'zh', 'ja', 'ko', 'ar'],
  ...
  static parsePublicScoreboardRoute(
    ...
    } else if (validationRules.validLanguages && !validationRules.validLanguages.includes(language)) {
    ...
    } else if (validationRules.validGameTypes && !validationRules.validGameTypes.includes(gameType)) {
...
static parseSearchParams(searchParams: URLSearchParams | ReadonlyURLSearchParams): Record<string, string> {
  ...
  static validateTeamId(teamId: string | number): { isValid: boolean; teamId?: number; error?: string } {
  ...
  static extractParamsFromPath(pathname: string): RouteParams | null {
    const match = pathname.match(/^\/public-scoreboard\/([^\/]+)\/(\d+)\/([^\/]+)\/([^\/]+)\/([^\/]+)\/?$/);
